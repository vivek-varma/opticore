...
class TestPricingAccuracy:
    ...
    def test_call_price(self, case):
        ...
        np.testing.assert_allclose(
            price, c["call_price"], rtol=1e-10, err_msg=f"{c['desc']} call price"
        )
    ...
    def test_put_price(self, case):
        ...
        np.testing.assert_allclose(
            price, c["put_price"], rtol=1e-10, err_msg=f"{c['desc']} put price"
        )
...
class TestGreeksAccuracy:
    ...
    def test_call_greeks(self, case):
        ...
        np.testing.assert_allclose(
            g.price, c["call_price"], rtol=1e-10, err_msg=f"{c['desc']} call price"
        )
        np.testing.assert_allclose(
            g.delta, c["call_delta"], rtol=rtol, err_msg=f"{c['desc']} call delta"
        )
        np.testing.assert_allclose(g.gamma, c["gamma"], rtol=rtol, err_msg=f"{c['desc']} gamma")
        np.testing.assert_allclose(g.vega, c["vega"], rtol=rtol, err_msg=f"{c['desc']} vega")
        np.testing.assert_allclose(
            g.theta, c["call_theta"], rtol=rtol, err_msg=f"{c['desc']} call theta"
        )
        np.testing.assert_allclose(g.rho, c["call_rho"], rtol=rtol, err_msg=f"{c['desc']} call rho")
    ...
    def test_put_greeks(self, case):
        ...
        rtol = self.GREEKS_RTOL
        np.testing.assert_allclose(
            g.price, c["put_price"], rtol=1e-10, err_msg=f"{c['desc']} put price"
        )
        np.testing.assert_allclose(
            g.delta, c["put_delta"], rtol=rtol, err_msg=f"{c['desc']} put delta"
        )
        np.testing.assert_allclose(g.gamma, c["gamma"], rtol=rtol, err_msg=f"{c['desc']} gamma")
        np.testing.assert_allclose(g.vega, c["vega"], rtol=rtol, err_msg=f"{c['desc']} vega")
        np.testing.assert_allclose(
            g.theta, c["put_theta"], rtol=rtol, err_msg=f"{c['desc']} put theta"
        )
        np.testing.assert_allclose(g.rho, c["put_rho"], rtol=rtol, err_msg=f"{c['desc']} put rho")
...
class TestIVAccuracy:
    ...
    def test_call_iv_round_trip(self, case):
        ...
        if c["sigma"] >= 0.02:
            np.testing.assert_allclose(
                solved, c["sigma"], rtol=1e-12, err_msg=f"{c['desc']} call IV"
            )
        else:
            np.testing.assert_allclose(
                solved, c["sigma"], atol=1e-9, err_msg=f"{c['desc']} call IV (low vol)"
            )
    ...
    def test_put_iv_round_trip(self, case):
        ...
        if c["sigma"] >= 0.02:
            np.testing.assert_allclose(
                solved, c["sigma"], rtol=1e-12, err_msg=f"{c['desc']} put IV"
            )
        else:
            np.testing.assert_allclose(
                solved, c["sigma"], atol=1e-9, err_msg=f"{c['desc']} put IV (low vol)"
            )
...
class TestArrayHandling:
    ...
    def test_0d_array_treated_as_scalar(self):
        ...
        p = oc.price(
            ...
            strike=np.float64(100.0),
            expiry=np.float64(1.0),
            rate=0.05,
            vol=0.2,
            kind="call",
        )
    ...
    def test_kind_variations(self):
        ...
        for k in ("put", "Put", "PUT", "p", "P"):
            p = oc.price(spot=100, strike=100, expiry=1.0, rate=0.05, vol=0.2, kind=k)
            assert p == base_put
